...
- **Rust hot-path rewrite** (`rust/`) — full feature parity except OpenTelemetry
  tracing: data-plane core + axum/reqwest streaming proxy + Prometheus metrics
  + circuit breaker + failover + tenancy (RPS/TPS limits, 429 + Retry-After +
  X-RateLimit-*, per-tenant prefix isolation via `cache_salt`) + structured
  JSON logging (request_id + duration_ms), plus fast Rust mock + Rust loadgen.
  **45 Rust unit tests** all passing. **Measured (see [BENCHMARKS.md](BENCHMARKS.md))**:
  full-parity Rust c=64 **10,922 req/s** vs Python 209 (~52× higher), p99
  **12.20 ms** vs 399.5 ms (~33× lower), +0.8 ms added latency at c=1 vs
  Python's +3.8 ms (~4.5× lower).
