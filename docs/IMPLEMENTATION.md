...
- **Rust hot-path rewrite** — in progress (`rust/`): port the data-plane core
  (hashing, radix tree, router) then the Tokio/hyper streaming proxy, for
  profiled before/after latency vs this Python baseline.
